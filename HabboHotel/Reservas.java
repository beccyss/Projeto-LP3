...
public class Reservas {
    ...
    private long calcularDias(){
        try {
            ...
            // Adicionado 1 para incluir o dia do check-in na contagem
            return ChronoUnit.DAYS.between(checkIn, checkOut) + 1;
        } catch (Exception e) {
            System.err.println("Erro ao calcular dias da reserva: " + e.getMessage());
            return 1; // Retorna 1 dia como padrão em caso de erro
        }
    }

    // Novo método para formatar a reserva para escrita em arquivo
    public String toFileString() {
        // Formato: CPF_Hospede;Nome_Hospede;Tipo_Quarto;Num_Pessoas_Quarto;CheckIn;CheckOut;ValorTotal;FormaPagamento;
        return hospede.getCpf() + ";" +
               ...
               dataCheck_in + ";" +
               dataCheck_out + ";" +
               String.format("%.2f", valorTotal).replace(",", ".") + ";" + // Usa ponto para decimal para facilitar leitura em outros sistemas
               formaPagamento + ";";
    }

    // Classes aninhadas que você tinha, considere refatorar para métodos em uma classe de serviço.
    // Estão aqui apenas para manter a estrutura original.
    ...
    public class statusReserva { } // Confirmada, pendente, cancelada;
}